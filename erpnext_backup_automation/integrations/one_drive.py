...
def upload_to_onedrive(file_path, rclone_remote_name=None):
    try:
        site_conf = _get_site_conf()

        rclone_remotes = site_conf.get("rclone_remotes", {})
        rclone_remote_name = rclone_remote_name or "onedrive"
        ...
        log_backup_update_last(
            one_drive_status="Success"
        )
        return remote_target
    except Exception as e:
        log_backup_update_last(
            one_drive_status="Failed",
            error_message=str(e)
        )
        frappe.log_error(frappe.get_traceback(), "OneDrive Backup Failed")  
